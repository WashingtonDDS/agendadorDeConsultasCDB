...
public class SecretariaRepository implements SecretariaOutputPort {
    ...
    final RowMapper<Secretaria> secretariaRowMapper = (rs, rowNum) -> {
    ...
    RowMapper<Secretaria> getSecretariaRowMapper() {
        return this.secretariaRowMapper;
    }
    ...
    public Secretaria findById(UUID id) {
        ...
        try {
            ...
            throw new RuntimeException("Recurso não encontrado: Secretaria com id " + id);
    ...
    public void delete(Secretaria secretaria) {
        if (secretaria == null || secretaria.getId() == null) {
            logger.warn("Tentativa de deletar secretaria com objeto ou ID nulo.");
            throw new IllegalArgumentException("Secretaria ou ID da secretaria não pode ser nulo para deleção.");
        }
